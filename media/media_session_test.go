...
func TestMediaPortRange(t *testing.T) {
	...
	alloc := getPortAllocator()
	require.NotNil(t, alloc)
	require.Equal(t, 5, alloc.InUse())

	// All 5 slots used - next allocation must fail
	_, err := NewMediaSession(net.IPv4(127, 0, 0, 1), 0)
	require.Error(t, err)

	// Close all sessions - ports should be released back
...
func TestMediaPortRangeConcurrent(t *testing.T) {
	RTPPortStart = 6000
	RTPPortEnd = 6100 // 50 slots

	const n = 50
	results := make([]*MediaSession, n)
	errs := make([]error, n)
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			defer wg.Done()
			m, err := NewMediaSession(net.IPv4(127, 0, 0, 1), 0)
			results[i] = m
			errs[i] = err
		}(i)
	}
	...
	alloc := getPortAllocator()
	require.Equal(t, n, alloc.InUse())

	// Verify all ports are unique
	ports := map[int]bool{}
	for _, s := range results {
		require.False(t, ports[s.Laddr.Port], "duplicate port %d", s.Laddr.Port)
		ports[s.Laddr.Port] = true
	}

	// Close all and verify release
	for _, s := range results {
		s.Close()
	}
	require.Equal(t, 0, alloc.InUse())
...
func TestMediaPortRangeWrapAround(t *testing.T) {
	RTPPortStart = 7000
	RTPPortEnd = 7010 // 5 slots

	// Fill all slots
	sessions := make([]*MediaSession, 5)
	for i := 0; i < 5; i++ {
		m, err := NewMediaSession(net.IPv4(127, 0, 0, 1), 0)
		require.NoError(t, err)
		sessions[i] = m
	...
	// Release the first two (ports 7000, 7002)
	sessions[0].Close()
	sessions[1].Close()

	// New allocations should wrap around and reuse the freed ports
	m1, err := NewMediaSession(net.IPv4(127, 0, 0, 1), 0)
	require.NoError(t, err)
	m2, err := NewMediaSession(net.IPv4(127, 0, 0, 1), 0)
	require.NoError(t, err)
