...
type MediaSession struct {
	...
	// portRelease returns the allocated port to the bitmap allocator on Close.
	// Transferred (not copied) by Fork so only the active session releases.
	portRelease func()
...
func (s *MediaSession) Fork() *MediaSession {
	cp := MediaSession{
		...
		DTLSConf:       s.DTLSConf,
		portRelease:    s.portRelease,
	}
	s.portRelease = nil // transfer ownership to fork
	return &cp
}
...
// createListeners sets up RTP and RTCP UDP listeners.
// When a port range is configured (RTPPortStart/RTPPortEnd), it uses a bitmap
// allocator for O(1) amortized, contention-free port assignment with proper
// wrap-around and release on close.
func (s *MediaSession) createListeners(laddr *net.UDPAddr) error {
	if laddr.Port != 0 {
		return s.listenRTPandRTCP(laddr)
	}

	if alloc := getPortAllocator(); alloc != nil {
		var lastErr error
		for attempts := 0; attempts < alloc.size; attempts++ {
			port, err := alloc.Allocate()
			if err != nil {
				return err // all slots in use
			}
			laddr.Port = port
			if err := s.listenRTPandRTCP(laddr); err != nil {
				alloc.Release(port)
				lastErr = err
				continue
			}
			s.portRelease = func() { alloc.Release(port) }
			return nil
		}
		return fmt.Errorf("no bindable ports in range %d:%d: %w", RTPPortStart, RTPPortEnd, lastErr)
	}

	// No port range configured - let OS assign ephemeral ports.
	// Retry because we need consecutive even (RTP) + odd (RTCP) ports.
	var err error
...
func (s *MediaSession) listenRTPandRTCP(laddr *net.UDPAddr) error {
	...
	if RTPDebug {
		DefaultLogger().Debug(fmt.Sprintf("[SETUP] Listening RTP %s", s.rtpConn.LocalAddr().String()))
		DefaultLogger().Debug(fmt.Sprintf("[SETUP] Listening RTCP %s", s.rtcpConn.LocalAddr().String()))
	}
